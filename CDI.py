...
def run(model_type,data_path, epochs, lr, diffusion_steps):
    ...
    samples.to_csv(results_path+'/samples_number.csv', index=False)
    ...
    samples.to_csv(results_path+'/samples_value.csv', index=False)
    ...
    Column_Shapes, Column_Pair_Trends = svd_evaluation(real_data=df, synthetic_data=samples, savepath=results_path+'/svd_evaluation', show=True)
